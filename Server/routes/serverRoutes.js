...
router.get("/userHome/:user/:title", (req, res) => {
	...
	db.getUserData(userName, { layout: false })
		.then((userDetails) => {
			db.getItineraries(userDetails.username).then((itineraryList) => {
...
router.post("/userHome/:user", (req, res) => {
	...
	if (req.body.city === "y") {
		request
			...
			.then((data) => {
				db.getUserData(username).then((userDetails) => {
					db.getItineraries(userDetails.username).then((itineraryList) => {
						res.render("itinerary", {
							layout: false,
							user: userDetails,
...
router.post("/signin", (req, res) => {
	...
	db.getPW(username).then((PW) => {
		if ((username === "") | (password === "")) {
			...
			res.render("signin", { layout: false, validation: "*User not found" });
		} else if (PW.password !== password) {
			res.render("signin", {
				layout: false,
				validation: "*Incorrect username or password",
			});
		} else {
